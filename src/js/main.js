...
function doneLoading() {
    ...
    gray.desaturate();
    const maxIt = 150; // Max iterations in spin
    // Creating menu
    ...
    function changeBalance(changeBy) {
        bal = bal + changeBy;
        let balS = bal.toFixed(2); // String with 2 digits after the decimal point
        let balInt = bal.toFixed(0); // String with no digits after the decimal point
        let output = "";
    ...
    plus.on('mousedown', function (e) {
        if (bet.text == bets[bets.length - 1].toString()) {
            ...
            minus.filters = [];
            let temp = bets.indexOf(parseFloat(bet.text));
            let temp2 = bets[temp + 1];
